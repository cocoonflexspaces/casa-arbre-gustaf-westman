#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────
# deploy.sh — push the Casa Arbre × Gustaf Westman deck to GitHub Pages
#
# What this does:
#   1. Initializes a git repo in this folder (if not already)
#   2. Commits everything
#   3. Creates a NEW public GitHub repo under your account
#   4. Pushes the deck
...
DEFAULT_REPO_NAME="cocoon-gustaf-westman-pop-up"
...
if ! git diff --cached --quiet; then
  git commit -q -m "Casa Arbre × Gustaf Westman — pop-up deck"
fi
...
# Pages is now public-only on free accounts; flip repo to public if needed
